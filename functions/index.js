const functions = require("firebase-functions"); //Cloud fonksiyonları oluşturmayı ve nasıl tetikleneceğinin belirlenmesini sağlar.

const admin = require('firebase-admin');//Firebase servislerindeki verileri yönetmeyi sağlar.
admin.initializeApp();
...
exports.kayiGuncellendi = functions.firestore.document('deneme/{docId}').onUpdate((change, context) => {
    admin.firestore().collection("gunluk").add({
        "aciklama": "Deneme koleksiyonundan kayıt güncellendi."
    });
});
